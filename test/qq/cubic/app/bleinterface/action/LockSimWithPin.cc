/**
 * @file LockSimWithPin.cc
 * @author Lele.Zhou
 * @version 1.0
 */
#ifndef _LOCKSIMWITHPIN_CC_
#define _LOCKSIMWITHPIN_CC_ 1
...
#undef CUBIC_LOG_TAG
#endif //CUBIC_LOG_TAG
#define CUBIC_LOG_TAG "LockSimWithPin"

using namespace std;
using namespace rapidjson;

#define DEF_PIN_KEY "pin"
class LockSimWithPin: public CDataObject
{
private:
    string method;
    string pin;
...
public:
    ...
    virtual int handler( const char *json_req, int size_req, char *json_rsp, int size_rsp )
    {
        ...
        cubic_msg_net_enable_pin enablePin;
        ...
        if( n_ret == 200 )
        {
            memset( &enablePin, 0, n_len );
            n_len = ( CUBIC_PIN_PUK_LEN_MAX > pin.length() ) ? pin.length() : CUBIC_PIN_PUK_LEN_MAX - 1;
            memcpy( enablePin.pin_puk_code, pin.c_str(), n_len );

            if( CubicPostReq( CUBIC_APP_NAME_NET_SERVICE, CUBIC_MSG_NET_ENABLE_PIN, enablePin ) == 0 )
            {
                str_ret = "OK";
                n_ret = 200;
            ...
            {
                n_ret = 400;
                str_ret = "event 'CUBIC_MSG_NET_ENABLE_PIN' send error";
            }
        }

        method = "LockSimWithPin";
        return createResultJson( method, n_ret, str_ret.c_str(), json_rsp, size_rsp );
    };
};

REGISTER_PRODUCTOR_NAME( LockSimWithPin, CDataObject, CDATA_OBJECT_LOCKSIMWITHPIN, DATA_OBJECT_NAME_LOCKSIMWITHPIN )

#endif //_LOCKSIMWITHPIN_CC_
