...
/**
 * Created by chenMeng on 2017/12/4.
 */
public class CrashHandler implements UncaughtExceptionHandler {
    ...
    private boolean handleException(Throwable ex) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                Looper.prepare();
                ToastUtil.showBottomShort("Program exception, exit");
                Looper.loop();
            }
        }).start();
...
//        sendErrorLog(path);
    ...
    private void sendErrorLog(String path) {
        LogUtil.e("result","sendErrorLog");
        LogUtil.e("result","path:"+path);
    }
    ...
    private void saveErrorLog() {
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            ...
            String path = Environment.getExternalStorageDirectory().getPath()+"/"+ RequestCode.UPLOAD_SAVE_FILE_NAME+"/";
    ...
    private void collectCrashInfo(Throwable ex) {
        ...
        LogUtil.d(TAG, "saveCrashInfo2File:" + mErrorLogBuffer.toString());
    }
