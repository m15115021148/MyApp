...
/**
 * Created by chenmeng on 2017/12/18.
 */
public class CustomDatePicker {
    ...
    public interface ResultHandler {
        void handle(String time,int type);
    }
    ...
    public CustomDatePicker(Context context, ResultHandler resultHandler, int off) {
        canAccess = true;
        this.context = context;
        this.handler = resultHandler;
        selectedCalender = Calendar.getInstance();
        ...
        int startYear = mYear - off;
        int endYear = mYear + off;
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.CHINA);
        try {
            startCalendar.setTime(sdf.parse(startYear+"-01-01 00:00"));
            endCalendar.setTime(sdf.parse(endYear+"-12-31 23:59"));
        } catch (ParseException e) {
            e.printStackTrace();
        }
        initDialog();
    ...
    private void initView() {
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm", Locale.CHINA);
                handler.handle(sdf.format(selectedCalender.getTime()),type);
                datePickerDialog.dismiss();
            }
        });
    }
    ...
    public void show(int type) {
        ...
        if (startCalendar.getTime().getTime() < endCalendar.getTime().getTime()) {
            ...
            initTimer();
            addListener();
            setSelectedTime(format.format(Calendar.getInstance().getTime()));
